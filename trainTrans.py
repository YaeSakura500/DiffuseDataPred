...
def train(model, param, epoch, devices:Union[str,List[int]],mu=1, ga=0.1, data_num=108,batch_size=27,new_data=False,RF=False):
    ...
    model=data_parallel.BalancedDataParallel(0,module=model,device_ids=device_ids[2:],output_device=device_ids[2])  
    encoder=data_parallel.BalancedDataParallel(12,module=encoder,device_ids=[0,1],output_device=device_ids[2])
    # decoder=data_parallel.BalancedDataParallel(2,module=decoder,device_ids=[5,6,7],output_device=device_ids[7])
    model=model.to(device_ids[2])
...
ep=0
for i in range(30):
    train('Trans_gen',param=[9216,16,2],epoch=30,batch_size=5,devices='all',RF=False)
# depth=[4,6,8,10,12]
# for i in depth:
#     train('Rcn',param=[i,3,3*16,3*16,3*16,True],epoch=10,batch_size=54,devices='all',RF=False)
